...
from DataAugmentatorV2 import DataAugmentatorV2;
...
class ModelTrainerV2():
    ...
    def train_es(self,model,tuning=True): #Runs training with early stopping. When tuning=true uses tuning test set, otherwise uses final test set.
        ...
        X_train_cupy=cp.array(X_train);#Converts to cupy for optimized augmentation
        #ipdb.set_trace();
        for n_epoch in range(self.n_epochs_max):
            print("=== Epoch:", n_epoch,"===")
            #Augmentation plus timing
            start_time = time.time()
            X_train_aug=self.da.augment_data(X_train_cupy); #Augments the data
            X=np.asarray(X_train_aug); #Converts back to numpy
