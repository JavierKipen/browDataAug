...
def quipu_df_to_numpy(df): # dataframe data structure to numpy arrays, and barcodes in onehot encoding.
    ...
    Y = np.vstack( df.antibody.values ) #1D binary signal in this case
    return X_numpy,Y; 
...
def divide_numpy_ds(X,Y,prop,keep_perc_classes=False,repeat_classes=False): #Divides train in train and validation. Prop indicates proportion of train ds
    ...
    if (keep_perc_classes == False):
        ...
        #ipdb.set_trace(); 
        ids_present_antibody=np.argwhere(Y==1)[:,1] ##Only to check classes
        ids_not_present_antibody=np.argwhere(Y==0)[:,1] ##Only to check classes
...
def allDataset_loader(data_folder,path_dataset_preprocessed="../../data/dataset.hdf5",cut=True):
    ...
    if os.path.exists(path_dataset_preprocessed):
        ...
        quipu_ds=allDatasets[ ["Antibodies", "nanopore","trace"]]; #Keep the only information that we will use
        quipu_ds.to_hdf(path_dataset_preprocessed, 'datasetQuipu');
        allDatasets.trace = traces_normalised;
        uncut_ds=allDatasets[ ["Anitbodies", "nanopore","trace"]]; #Keep the only information that we will use
