...
import ipdb
from DataLoading import get_dataset_as_Quipu,get_datasets_numpy
...
class ModelTrainer():
    def __init__(self,n_epochs_max=100,lr = 1e-3,batch_size=128,early_stopping_patience=18,brow_std=0.9,brow_aug_use=True,opt_aug=False,use_weights=False,track_losses=False, optimizer="Adam",momentum=None,check_stretch_brow=False): #Opt_aug still has bugs, have to check
        ...
        self.df_train,self.df_test=get_dataset_as_Quipu(QUIPU_DATA_FOLDER)
        self.shapeX = (-1, QUIPU_LEN_CUT,1); self.shapeY = (-1, 1);
    ...
    def train_es(self,model, batch_size_val=512,sameQuipuTestSet=False): #Runs training with early stopping, more controlled manner than quipus original
        ...
        X_train,X_valid,Y_train,Y_valid,X_test,Y_test= get_datasets_numpy(self.df_train,self.df_test,repeat_classes= (not self.use_weights) ,sameQuipuTestSet=sameQuipuTestSet); #When weights are used 
