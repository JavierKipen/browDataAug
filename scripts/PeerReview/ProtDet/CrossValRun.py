...
from Models import YupanaNetModif
...
def crossval_run_w_notes(modelTrainer,model_base,out_folder,comment="",title_file="",n_runs=10,tuning=False,save_model=True):
    str_time=datetime.today().strftime('%Y%m%d_%H-%M-%S');
    log_path=out_folder+str_time+"_"+title_file+"_log.txt";
    best_test_acc=0;
    
    with open(log_path, 'w') as f:
        model_base.summary(print_fn=lambda x: f.write(x + '\n'))
    ...
    for i in range(n_runs):
        ...
        init_data = [train_acc, valid_acc, test_acc]
        df_results = pd.DataFrame([init_data], columns=["train_acc","valid_acc","test_acc"]);
...
if __name__ == "__main__":
    ...
    mt=ModelTrainerV2(lr = 2e-4);
    model=YupanaNetModif();
    
    mt.n_epochs_max=60;
    n_runs=20;
    out_folder="../../../results/PeerReview/ProtDet/";
    
    crossval_run_w_notes(mt,model,out_folder,comment="Running prot detection w yupananet",title_file="YupanaNetProtDet",n_runs=n_runs)
