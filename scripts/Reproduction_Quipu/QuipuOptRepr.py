...
subfolder_res= "Reproduction/QuipuOptRepr/";
...
mt=ModelTrainer(brow_aug_use=False,n_epochs_max=n_epochs,early_stopping_patience=n_epochs,brow_std=0.0,batch_size=256);
model_base=get_quipu_model();

for i in range(n_runs):
    model=clone_model(model_base);
    
    train_acc, valid_acc, test_acc, n_epoch = mt.train_es(model,sameQuipuTestSet=True);
    str_time=datetime.today().strftime('%Y%m%d_%H-%M-%S');
    run_name=str_time+"_QuipuOptRepr_Ne_"+str(n_epochs) + ".csv";
